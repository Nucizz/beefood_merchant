    ...
    <div className="relative z-0 w-full">
      <div className="relative">
        <select
          ...
          value={value}
          className={
            "cursor-pointer w-full h-12 md:text-base text-sm pt-3 md:pb-2 block px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black dark:focus:border-white border-gray-200 dark:border-gray-400 rounded-none " +
        ...
        <label
          htmlFor={name}
          className={
            "absolute top-5 md:top-3 -z-1 origin-0 text-gray-500 dark:text-gray-300 md:text-base text-sm w-full cursor-pointer " +
            (isFilled ? "dropdown-label" : "")
          }
        >
          {label}
        </label>
...
export function TextField({
  ...
  return (
    <div className="relative z-0 w-full">
      ...
      <label
        htmlFor={name}
        className="absolute duration-300 top-5 md:top-3 -z-1 origin-0 text-gray-500 dark:text-gray-300 md:text-base text-sm w-full cursor-text"
      >
        {label}
      </label>
      {type === "password" && (
  ...
  return (
    <div className="relative z-0 w-full">
      <textarea
        ...
        placeholder=" "
        className={
          "h-32 md:text-base text-sm pt-4 md:pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black dark:focus:border-white border-gray-200 dark:border-gray-400 rounded-none " +
          (disabled
            ? "text-gray-500 dark:text-gray-300"
            : "text-black dark:text-white")
        }
      ...
      <label
        htmlFor={name}
        className="absolute duration-300 top-4 -z-1 origin-0 text-gray-500 dark:text-gray-300 md:text-base text-sm w-full cursor-text"
      >
        {label}
      </label>
    </div>
  ...
  return (
    <div className={"flex flex-col gap-4 " + classSize}>
      ...
      <label
        htmlFor="fileInput"
        className={
          (!disabled ? "cursor-pointer" : "hidden") +
          " text-transparent hover:bg-black/25 hover:text-white text-semibold transition-all duration-300 absolute flex items-center justify-center rounded-full " +
          classSize
        }
      >
        Change Photo
...
export function Accordion({
  ...
  return (
    <div>
      <div
        className={
          "group flex items-center justify-between cursor-pointer lg:px-6 lg:py-4 md:px-4 md:py-3 px-3 py-2 rounded-lg transition-all duration-300 hover:bg-amber-400 hover:text-white " +
          (index === indexId
      ...
      {index === indexId && (
        <div
          className={`text-black dark:text-white bg-white dark:bg-slate-800 p-4 rounded-lg mt-2 ${index === indexId ? "h-fit" : "h-0"
            }`}
        >
          {Children}
        </div>
...
export function Toast({ message, setShowRef }) {
  ...
  return (
    <div
      class="fixed ml-4 mt-4 md:ml-72 md:mt-8 lg:ml-76 lg:mt-12 inset-0 h-fit z-30 m-4 bg-white dark:bg-slate-800 dark:text-white text-black rounded-md shadow-lg border-2 border-amber-500 flex flex-col px-4 py-2 lg:w-1/3 md:w-1/2 w-full"
      role="alert"
    >
      <div className="flex flex-row w-full justify-between">
        <h3 className="font-semibold text-amber-500 text-lg">
          {message.title}
        </h3>
        <button
