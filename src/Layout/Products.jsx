...
import {
    addProduct,
    changeProductAvailbility,
    deleteProduct,
    getProductList,
    updateProduct,
} from "../Javascript/ProductHandler";
...
export default function ProductsLayout({ merchanRef }) {
    ...
    return (
        <div className="w-full gap-4 flex flex-col">
            <ProductLayoutHeader
                setProductListRef={setFilteredProducts}
                merchanRef={merchanRef}
                searchQueryRef={searchQuery}
                setSearchQueryRef={setSearchQuery}
            ...
            <ProductList
                setProductListRef={setFilteredProducts}
                productListRef={filteredProducts}
                merchanRef={merchanRef}
            />
...
function ProductLayoutHeader({
    ...
    return (
        <div className="w-full flex flex-col lg:flex-row items-start lg:justify-between">
            <h1 className="w-fit text-3xl font-bold leading-none tracking-tight text-black dark:text-white md:text-4xl">
                Products
            </h1>
            ...
            <div className="lg:w-2/3 xl:w-1/2 w-full flex flex-row gap-5 items-center justify-between">
                ...
                />
                <button
                    className="w-52 bf-bg-color md:h-9 h-8 rounded-md font-medium text-white mt-5"
                    type="button"
                    onClick={() => setAddProductOverlay(true)}
                >
                    Add Product
                </button>
            </div>
            ...
            {addProductOverlay ? (
                <AddProduct
                    setProductListRef={setProductListRef}
                    setAddProductOverlayRef={setAddProductOverlay}
                    merchanRef={merchanRef}
        ...
        <>
            <table className="w-full table-auto rounded-lg overflow-hidden xl:text-lg lg:text-base text-sm">
                <thead className="bg-white dark:bg-slate-800 border-b ">
                    <tr>
                        <th
                            className="p-2 lg:pl-4 text-left font-semibold text-black cursor-pointer group transition-all duration-300 hover:text-gray-500 dark:text-gray-300"
                            onClick={() => handleSort("name")}
                        >
                            Name{" "}
                            <span className="text-black dark:text-white rounded-full ml-2 transition-all duration-300 group-hover:text-gray-500">
                                {sortBy === "name" && (sortProduct === "asc" ? "▲" : "▼")}
                            </span>
                        </th>
                        <th
                            className="p-2 text-left font-semibold text-black cursor-pointer group transition-all duration-300 hover:text-gray-500 dark:text-gray-300"
                            onClick={() => handleSort("price")}
                        >
                            Price{" "}
                            <span className="text-black dark:text-white rounded-full ml-2 transition-all duration-300 group-hover:text-gray-500">
                                {sortBy === "price" && (sortProduct === "asc" ? "▲" : "▼")}
                            </span>
                        </th>
                        <th
                            className="p-2 text-left font-semibold text-black cursor-pointer group transition-all duration-300 hover:text-gray-500 dark:text-gray-300"
                        ...
                        >
                            Sales{" "}
                            <span className="text-black dark:text-white rounded-full ml-2 transition-all duration-300 group-hover:text-gray-500">
                                {sortBy === "totalSale" && (sortProduct === "asc" ? "▲" : "▼")}
                            </span>
                        </th>
                ...
                <tbody>
                    {sortedProductListRef.length > 0 ? (
                        sortedProductListRef.map((product) => (
                            ...
                            >
                                <td className="p-2 cursor-pointer lg:pl-4 text-left text-black dark:text-white flex flex-row gap-2 lg:gap-5 items-center">
                                    <img
                                        src={product.thumbnailPicture ?? BeeFood}
                                        className="border w-10 h-10 lg:w-16 lg:h-16 rounded-lg object-cover"
                                        alt={product.name}
                                    />
                                    <span className="max-h-10 lg:max-h-16 overflow-hidden text-ellipsis">
                                        {product.name}
                                    </span>
                                </td>
                                <td className="p-2 cursor-pointer text-left text-black dark:text-white">
                                    {moneyConverter(product.price)}
                                </td>
                                <td className="p-2 cursor-pointer text-left text-black dark:text-white">
                                    {product.totalSale}
                                </td>
                                {window.innerWidth >= 768 ? (
                                    <td
                                        className={
                                            "p-2 cursor-pointer text-left font-semibold " +
                        ...
                        <tr>
                            <td
                                colSpan="4"
                                className="p-2 text-center bg-white dark:bg-slate-800 "
                            >
            ...
            {selectedProduct ? (
                <ProductDetails
                    ...
                    setSelectedProductRef={setSelectedProduct}
                    merchanRef={merchanRef}
                />
            ) : null}
        </>
...
function ProductDetails({
    ...
    const onDeleteProduct = async () => {
        if (await deleteProduct(merchanRef.id, product.id)) {
            const updatedProducts = merchanRef.product.filter(
                (p) => p.id !== product.id
            );
        ...
        <div className="z-30 fixed inset-0 flex items-center justify-center bg-black/50">
            <div className="md:ml-64 w-full mx-10 md:mx-0 md:w-1/2 bg-white dark:bg-slate-800 md:p-4 p-3 rounded-md flex flex-col lg:gap-4 md:gap-3 gap-2">
                <div className="flex flex-row justify-between items-center text-xl md:text-2xl">
                    <span className="w-fit font-bold leading-none tracking-tight text-black dark:text-white ">
                        Product Details
                    </span>

                    <button
                        class="transition-all duration-300 text-gray-500 dark:text-gray-300 hover:text-gray-600 dark:hover:text-gray-600 font-bold"
                ...
                <form className="grid grid-cols-1 gap-3">
                    <div className="w-full flex flex-col lg:flex-row items-center justify-evenly lg:gap-0 gap-5">
                        <ChangePhoto
                            ...
                            setPhotoRef={setThumbnail}
                            classSize={"xl:w-32 xl:h-32 w-24 h-24"}
                            disabled={!editable}
                        />
                        <div className="flex flex-col items-center justify-evenly w-full lg:w-1/3">
                            <span className="rounded-md bg-amber-200 text-amber-500 dark:bg-amber-600 dark:text-amber-100 py-1 px-2 md:py-2 md:px-3 w-full h-fit lg:text-base text-sm">
                                This product was sold <b>{product.totalSale}</b> times.
                            </span>
                            <button
                                className={
                                    "w-full md:h-9 h-8 rounded-md font-medium text-white mt-5 transition-all duration-300 " +
                                    (available
                            ...
                            >
                                {"Product is " + (available ? "Available" : "Unavailable")}
                            </button>
                        </div>
                    </div>
                    {error ? (
                    ...
                    <TextField
                        ...
                        onChange={(e) => setPrice(validatePrice(e.target.value))}
                        disabled={!editable}
                    />
                    <LongTextField
                        label="Description"
                    ...
                    {!editable ? (
                        <div className="flex flex-row justify-end mt-5 gap-2">
                            <button
                                className="px-8 bg-red-500 hover:bg-red-600 md:h-9 h-8 rounded-md font-medium text-white transition-all duration-300"
                                type="button"
                            ...
                            <button
                                className="px-8 bf-bg-color md:h-9 h-8 rounded-md font-medium text-white"
                                type="button"
                ...
                {deleteConfirmation && (
                    <div className="z-30 fixed inset-0 flex items-center justify-center bg-black/50">
                        <div className="md:ml-64 bg-white dark:bg-slate-800 md:p-4 p-3 rounded-md flex flex-col lg:gap-4 md:gap-3 gap-2">
                            <p className="md:text-xl text-md font-semibold text-black dark:text-white">
                                Are you sure you want to permanently delete this product?
                            </p>
                            <div className="flex flex-row justify-end gap-2">
                                <button
                                    onClick={() => setDeleteConfirmation(false)}
                                    className="px-4 transition-all duration-300 bf-bg-color md:h-9 h-8 rounded-md font-medium text-white"
                                >
                                    Cancel
                                </button>
                                <button
        ...
        if (name === "" || price === "" || description === "" || !thumbnail) {
            ...
            if (res === "0") {
                ...
                setNotificationRef({
                    ...
                    body:
                        name +
                        " is now listed on your store for " +
                        moneyConverter(parseFloat(price.slice(3))) +
                        ". \r\n Happy cooking!",
                });
            } else {
                setError(res);
        ...
        <div className="z-30 fixed inset-0 flex items-center justify-center bg-black/50">
            <div className="md:ml-64 w-1/2 bg-white dark:bg-slate-800 md:p-4 p-3 rounded-md flex flex-col lg:gap-4 md:gap-3 gap-2">
                <div className="fle flex flex-row justify-between items-center text-xl md:text-2xl">
                    ...
                    <button
                        class="transition-all duration-300 text-gray-500 dark:text-gray-300 hover:text-gray-600 font-bold"
                        onClick={() => setAddProductOverlayRef(false)}
                    >
                        &#x2716;
                    </button>
                </div>
                ...
                <form className="grid grid-cols-1 gap-3">
                    <div className="w-full flex items-center justify-center">
                        <ChangePhoto
                            photoRef={thumbnail ? URL.createObjectURL(thumbnail) : null}
                            setPhotoRef={setThumbnail}
                            type="add"
