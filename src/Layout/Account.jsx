...
function ProfileInformation({ merchanRef, setMerchantRef }) {
    ...
    return (
        <div className="w-full flex flex-col lg:flex-row gap-4 lg:gap-10 lg:items-start items-center lg:h-36 mb-8">
            <ChangePhoto
                photoRef={merchanRef.profilePicture}
                classSize={"w-36 h-36"}
                disabled={true}
            />

            <div className="flex flex-col justify-evenly lg:items-start items-center h-full w-fit">
                <span className="xl:text-6xl md:text-4xl text-2xl font-bold text-center md:text-left text-black dark:text-white">
                    {merchanRef.name}
                </span>
                <span className="xl:text-2xl lg:text-xl md:text-lg text-md text-gray-500 dark:text-gray-300 text-center md:text-left">
                    {merchanRef.email}
                </span>
                <span className="xl:text-2xl lg:text-xl md:text-lg text-md text-gray-500 dark:text-gray-300 text-center md:text-left">
                    {merchanRef.phone}
            ...
            <button
                ...
                className="px-4 transition-all duration-300 bg-red-500 hover:bg-red-600 md:h-10 h-8 rounded-md font-medium text-white flex flex-row gap-2 items-center md:ml-auto"
            >
                <svg
                    className="lg:h-8 lg:w-8 md:w-6 md:h-6 w-5 h-5"
                    ...
                    <div className="md:ml-64 bg-white dark:bg-slate-800 md:p-4 p-3 rounded-md flex flex-col lg:gap-4 md:gap-3 gap-2">
                        ...
                        <div className="flex flex-row justify-end gap-2">
                            <button
                                ...
                                className="px-4 bf-bg-color md:h-9 h-8 rounded-md font-medium text-white"
                            >
                                Cancel
                            </button>
                            <button
...
function SettingsList({ merchanRef, setMerchantRef }) {
    ...
    return (
        <div className="w-full flex flex-col gap-4">
            <Accordion
                setAccordionRef={setOpenAccordion}
                index={openAccordion}
                indexId={1}
            ...
            <Accordion
                ...
                Children={<AccountSettings merchanRef={merchanRef} />}
            />
...
function AccountSettings({ merchanRef }) {
    ...
    const onResetPasswordPressed = () => {
        ...
        setConfirmationMessage(
            `An email has been sent to ${merchanRef.email} for password reset.`
        );
        setTimeout(() => {
    ...
    return (
        <div className="grid grid-cols-1 gap-3">
            <p className="rounded-md bg-amber-200 text-amber-500 dark:bg-amber-600 dark:text-amber-100 py-1 px-2 md:py-2 md:px-3 w-full h-fit lg:text-base text-sm">
                <b>Note:</b> To change your contact information such as email and phone
                please contact BeeFood admin.
            </p>
            <TextField
                label="Email"
                name="email"
                value={merchanRef.email}
                type="email"
                onChange={() => { }}
                disabled={true}
            />
            <TextField
                ...
                type="tel"
                onClick={() => { }}
                value={merchanRef.phone}
                onChange={() => { }}
                disabled={true}
            />

            <button
                className="ml-auto px-8 bg-red-500 hover:bg-red-600 md:h-9 h-8 transition-all duration-300 rounded-md font-medium text-white mt-5"
                type="button"
                    ...
                    <div className="md:ml-64 bg-white dark:bg-slate-800 md:p-4 p-3 rounded-md flex flex-col lg:gap-4 md:gap-3 gap-2">
                        ...
                        <div className="flex flex-row justify-end gap-2">
                            <button
                                ...
                                className={
                                    "px-4 bf-bg-color md:h-9 h-8 rounded-md font-medium text-white " +
                                    (buttonUsable ? "block" : "hidden")
                                }
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                className={
                                    "px-4 transition-all duration-300 bg-red-500 hover:bg-red-600 md:h-9 h-8 rounded-md font-medium text-white " +
                                    (buttonUsable ? "block" : "hidden")
                                }
                            >
                                Reset Password
    ...
    const updateGeneralData = async () => {
        if (location === "" || campus === "" || name === "") {
            ...
            const newProfilePicture =
                profilePicture !== merchanRef.profilePicture ? profilePicture : null;
            const res = await updateMerchantData(
                merchanRef.id,
                name,
                description,
                campus,
        ...
        <div className="grid grid-cols-1 gap-3">
            <div className="w-full flex md:flex-row flex-col items-center justify-evenly mb-5 md:gap-0 gap-5">
                <ChangePhoto
                    photoRef={
                        profilePicture
                            ...
                            : merchanRef.profilePicture
                    }
                    setPhotoRef={setProfilePicture}
                    classSize={"xl:w-32 xl:h-32 md:w-24 md:h-24 w-28 h-28"}
                    disabled={!editable}
                />
                <p className="rounded-md bg-amber-200 text-amber-500 dark:bg-amber-600 dark:text-amber-100 py-1 px-2 md:py-2 md:px-3 md:w-2/3 w-full h-fit lg:text-base text-sm">
                    <b>Note:</b> This changes applies right away and will be visible to
                    all of BeeFood merchants.
                </p>
            </div>
            {error ? (
                <div
                    className={
                        "mb-2 w-full md:h-9 h-8 rounded-md flex flex-row items-center md:px-3 px-2 md:text-base text-sm text-red-600 bg-red-100"
                    }
                >
                    {error}
                </div>
            ...
            {!editable ? (
                <button
                    className="ml-auto px-8 bf-bg-color md:h-9 h-8 rounded-md font-medium text-white mt-5"
                    type="button"
                ...
                <div className="flex flex-row justify-end mt-5 gap-2">
                    <button
                        className="px-8 bg-red-500 hover:bg-red-600 md:h-9 h-8 rounded-md font-medium text-white"
                        type="button"
                        onClick={onCancel}
                    >
                        Cancel
    ...
    return (
        <div className="grid grid-cols-1 gap-3">
            ...
            <p>
                ...
                support team at{" "}
                <a
                    href="mailto:beefood.contact@gmail.com"
                    className="bf-text-color hover:underline"
                >
