            ...
            {resetConfirmation && (
                <div className="z-30 fixed inset-0 flex items-center justify-center bg-black/50">
                    <div className="md:ml-64 bg-white md:p-4 p-3 rounded-md flex flex-col lg:gap-4 md:gap-3 gap-2">
                        ...
                        <div className="flex flex-row justify-end gap-2">
                            <button onClick={onResetPasswordPressed} className={"px-4 transition-all duration-300 bg-red-500 hover:bg-red-600 md:h-9 h-8 rounded-md font-medium text-white " + (buttonUsable ? "block" : "hidden")}>Confirm</button>
                            <button onClick={() => setResetConfirmation(false)} className={"px-4 transition-all duration-300 bg-gray-400 hover:bg-gray-500 md:h-9 h-8 rounded-md font-medium text-white " + (buttonUsable ? "block" : "hidden")}>Cancel</button>
                        </div>
                    </div>
                </div>
            )}
...
function GeneralSettings({merchant, setMerchantRef}) {
    ...
    const init = (data) => {
        ...
        setName(data.name)
        setDescritpion(data.description)
    ...
    return(
        <div className='grid grid-cols-1 gap-3'>
            ...
            <LongTextField label="Description" name="description" value={description} onChange={(e) => setDescritpion(e.target.value)} disabled={!editable} />
