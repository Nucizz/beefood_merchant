...
async function addFeedback(
    ...
    try {
        ...
        const existingSnapshot = await getDocs(
            query(feedbackDB, where("email", "==", email))
        );
    ...
    return (
        <div className="w-screen h-screen bg-black flex flex-row items-center justify-evenly bg-pattern">
            ...
            <div className="z-10 shadow-lg bg-white dark:bg-slate-800 rounded-lg text-black dark:text-white xl:block hidden">
                <iframe
                    title="figma"
                    className="rounded-lg xl:block hidden"
                    width="405"
                    height="720"
        ...
        <div className="w-full flex flex-col md:items-start items-center">
            ...
            <div className="flex md:flex-row flex-col items-center gap-5 mt-5">
                <p className="w-full text-gray-500 dark:text-gray-300 font-base lg:text-md md:text-lg text-base text-justify">
                    <span className="font-semibold">BeeFood</span> adalah sebuah aplikasi
                    layanan pemesanan makanan digital di area{" "}
                    <span className="font-semibold">BINUS</span> yang dibuat untuk{" "}
                    <span className="font-semibold">Mahasiswa</span>,{" "}
                    <span className="font-semibold">Dosen</span>,{" "}
                    <span className="font-semibold">Staff</span> serta{" "}
                    <span className="font-semibold">Tenant</span> yang berada di area
                    kampus <span className="font-semibold">BINUS</span>.
            ...
            <div className="w-full mt-2 text-gray-500 dark:text-gray-300 font-semibold lg:text-2xl md:text-xl text-lg text-center">
                Follow our Instagram{" "}
                <a
                    href="https://instagram.com/beefood_binus"
                    className="bf-text-color cursor-pointer"
                >
                    @beefood_binus
                </a>
            </div>
...
function FeedbackForm({ viewRef }) {
    ...
    const validateInstagram = (input) => {
        ...
        if (!input.startsWith("@")) {
            ...
            specialCharacters.some((char) => input.slice(1).includes(char))
        ) {
    ...
    return (
        <>
            ...
            <form className="grid grid-cols-1 gap-3">
                {error ? (
                    <div className="mb-2 w-full md:h-9 h-8 bg-red-100 rounded-md text-red-600 flex flex-row items-center md:px-3 px-2 md:text-base text-sm">
                        {error}
                    </div>
                ) : null}
                <TextField
                ...
                <TextField
                    ...
                    type="text"
                    value={instagram}
                    onChange={(e) => setInstagram(validateInstagram(e.target.value))}
                />
                <LongTextField
                    label="Apa yang menarik dari aplikasi BeeFood?"
                    name="positive"
                    type="text"
                    value={positiveFB}
                    onChange={(e) => setPositiveFB(e.target.value)}
                />
                <LongTextField
                    label="Apa yang kurang menarik dari aplikasi BeeFood?"
                    name="negative"
                    type="text"
                    value={negativeFB}
                    onChange={(e) => setNegativeFB(e.target.value)}
                />
                <LongTextField
                    label="Apa ada saran untuk BeeFood?"
                    name="tips"
                    type="text"
                    value={tipsFB}
                    onChange={(e) => setTipsFB(e.target.value)}
                />
                <button
                    className="w-full bf-bg-color md:h-9 h-8 rounded-md font-medium text-white mt-5"
                    type="button"
                    onClick={onFeedbackValidate}
                >
                    Send Feedback
                </button>
            </form>
    ...
    return (
        <div className="flex flex-col justify-center items-center gap-5">
            ...
            <p className="text-xl text-gray-500 dark:text-gray-300">
                Please take some freebies as a part of our appreciation.
            </p>
        </div>
    );
}
