...
import { getOrderList } from "../Javascript/OrderHandler";
...
export default function DashboardLayout({ merchanRef }) {
    ...
    const fetchTodayOrderList = async () => {
        try {
            setOrder(await getOrderList(merchanRef.id));
        } catch (e) {
            console.log(e);
        } finally {
            setIsLoading(false);
    ...
    return (
        <div className="w-full gap-4 flex flex-col">
            <h1 className="text-3xl font-bold leading-none tracking-tight text-black dark:text-white md:text-4xl lg:text-4xl">
                Dashboard
            </h1>
    ...
    return (
        <div className="w-full rounded-lg bg-white dark:bg-slate-800 lg:px-8 lg:py-6 md:px-6 md:py-4 px-4 py-2 flex lg:flex-row flex-col lg:items-start items-center lg:gap-8 gap-4">
            ...
            <div className="flex flex-col lg:items-start items-center">
                <span className="xl:text-6xl md:text-4xl text-2xl font-bold text-black dark:text-white text-center md:text-left">
                    {merchanRef.name}
                </span>
                <p className="xl:text-2xl lg:text-xl md:text-lg text-md text-gray-500 dark:text-gray-300 text-center md:text-left">
                    {merchanRef.description}
                </p>
            </div>
        </div>
    );
        ...
        <div className="w-full flex flex-col xl:gap-4 md:gap-3 gap-2">
            ...
            <ol className="w-full grid grid-cols-2 gap-y-5 lg:grid-cols-3 xl:grid-cols-5 rounded-lg py-4 bg-white dark:bg-slate-800">
                <li className="flex flex-col justify-center items-center w-full border-r-2 border-gray-200 dark:border-gray-400">
                    ...
                    <span className="text-sm md:text-base lg:text-lg font-semibold w-full text-center text-black dark:text-white">
                        Need Confirmation
                    </span>
                </li>
