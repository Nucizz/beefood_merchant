...
export function AdminMerchant({ page }) {
    ...
    return (
        <div className="w-screen h-screen bg-black flex items-center justify-center bg-pattern">
            ...
            <div
                className={
                    "z-10 shadow-lg lg:px-7 md:px-6 px-4 lg:py-6 md:py-5 py-4 bg-white dark:bg-slate-800 rounded-lg text-black dark:text-white xl:w-1/3 lg:w-2/5 md:w-1/2 w-full"
                }
            >
...
function MerchantRegisterForm() {
    ...
    const EMAIL_BODY =
        ...
        merchantName +
        "! \r\n\r\n" +
        "Congratulations, your request to be a part of BeeFood has been accepted! \r\n Please proceed to the registration form at " +
        LINK +
        "register and use this token " +
        token +
        " with the same email as well. \r\n\r\n" +
    ...
    return (
        <form className="grid grid-cols-1 gap-3">
            {error ? (
                <div
                    className={
                        "mb-2 w-full md:py-2 py-1 rounded-md flex flex-row items-center md:px-3 px-2 md:text-base text-sm " +
                        (error.startsWith("TOKEN: ")
            ...
            <a
                className={
                    "w-full font-medium bf-text-color hover:cursor-pointer text-center " +
...
function VerifyHalal() {
    ...
    const onVerifyHalal = async () => {
        ...
        if (res === "0") {
            setError(
                "Updated merhant as " + (!merchant.halal ? "halal." : "not halal.")
            );
            setMerchant(await getMerchantData(email));
        } else {
            setError(res);
            ...
            {!merchant ? (
                ...
                <>
                    ...
                    <JustifiedInfo
                        ...
                        content={merchant.halal ? "YES" : "NO"}
                    />
                    <button
                        className="w-full bf-bg-color md:h-9 h-8 rounded-md font-medium text-white mt-5"
                        type="button"
                        onClick={onVerifyHalal}
                    >
                        {merchant.halal
