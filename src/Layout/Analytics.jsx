...
import { getOrderList } from "../Javascript/OrderHandler.js";
import { OrderDetails } from "./Order.jsx";

export default function AnalyticsLayout({ merchanRef, setMerchantRef }) {
...
function MerchantSummary({ merchanRef, dailyEarningRef, dailyOrderRef }) {
    ...
    return (
        <div className="relative w-full rounded-lg bg-white dark:bg-slate-800 lg:px-8 lg:py-6 md:px-6 md:py-4 px-4 py-2 flex lg:flex-row flex-col items-center lg:gap-8 gap-4">
            <div className="lg:w-2/5 w-full flex flex-col items-start lg:border-r-2 border-gray-200 dark:border-gray-400">
                <span className="xl:text-2xl lg:text-xl text-lg font-bold text-gray-500 dark:text-gray-300 text-left">
            ...
            <div className="absolute top-0 right-0 p-4">
                <button
                    className="bf-bg-color xl:h-12 xl:w-12 h-10 w-10 rounded-md text-white flex items-center justify-center"
                    onClick={() => {
                        setViewAllTime(!viewAllTime);
                    }}
                >
                    <svg
                        viewBox="0 0 24 24"
                        className="xl:w-8 lg:h-8 h-7 w-7"
                        fill="none"
...
function DateList({ setDateRef, dateRef }) {
    const currentDate = new Date();
    const containerRef = useRef(null);

    const dateList = [...Array(14).keys()]
        .map((index) => {
        ...
        <div ref={containerRef} className="w-full overflow-x-auto mb-2">
            <div className="w-fit flex flex-row">
                {dateList.map((dateData) => (
                    <button
                        key={dateData}
                        onClick={() => setDateRef(dateData)}
                        className={
                            "date-button flex items-center justify-center h-12 w-12 rounded-md text-black dark:text-white font-semibold mr-5 transition-all duration-300 hover:bg-amber-400 dark:hover:bg-amber-400 " +
                            classAttributes(dateData.getDate())
                        }
                    >
                        {dateData.getDate()}
                    </button>
...
function TransactionListLayout({
    ...
    return (
        <div className="w-full flex flex-col xl:gap-4 md:gap-3 gap-2">
            <h2 className="w-full text-lg md:text-xl lg:text-2xl font-bold text-black dark:text-white">
                Transaction List
            </h2>

            <DateList setDateRef={setDateRef} dateRef={dateRef} />
